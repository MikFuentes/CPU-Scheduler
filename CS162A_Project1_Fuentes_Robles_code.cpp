...
string output(string algorithm, int numProcesses, int Q){
	...
	if (algorithm == "FCFS"){ //DONE	
		...
		while(numProcesses !=0){
			...
			// //check what's in the queue
			// for(process p : queue){
				// cout << p.arrivalTime << " " << p.burstTime << " " << p.burstTime << " " << p.index << endl;
			// }
			...
			if (!(queue.empty())) {
				...
				// cout << "top: " << queue.front().arrivalTime << " " << queue.front().burstTime << " " << queue.front().burstTime << " " << queue.front().index << endl;
				
				//if it doesn't need to run anymore (can be terminated)
				if(queue.front().burstTime <= 1){ //should be 1 or 0?
					// cout << "TERMINATE NEXT CC: " << queue.front().arrivalTime << " " << queue.front().burstTime << " " << queue.front().burstTime << " " << queue.front().index << endl;
							...
							for(process p : queue) {
								...
								for(int j = 0; j<queue.size()-1;j++){ 
									...
									//if the burstTime is equal to the current
										...
										//if the arrivalTime of the next process is less than the current
										if(queue.at(j+1).arrivalTime < queue.at(j).arrivalTime){
											//swap places
											swap(queue.at(j+1), queue.at(j));
										}
										//if the arrivalTimes are equal
										else if (queue.at(j+1).arrivalTime == queue.at(j).arrivalTime){
											//if the index of the next process is less than the current
